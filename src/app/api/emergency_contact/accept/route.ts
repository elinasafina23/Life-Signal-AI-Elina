...
export default function EmergencyDashboardPage() {
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      ...
      if (!user) {
        setLoading(false);
        router.replace(`/login?role=emergency_contact&next=${encodeURIComponent("/emergency-dashboard")}`);
        return;
      }
      ...
      try {
        ...
        if (myRole !== "emergency_contact") {
          if (inviteId && token) {
            router.replace(`/signup?role=emergency_contact&invite=${inviteId}&token=${token}`);
          } else {
